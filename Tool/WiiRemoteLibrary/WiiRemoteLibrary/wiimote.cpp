...
bool wiimote::Connect(unsigned wiimote_index, bool force_hidwrites)
{
	...
	while (SetupDiEnumDeviceInterfaces(dev_info, NULL, &guid, index, &didata))
	{
		...
		if (HidD_GetAttributes(Handle, &attrib))
		{
			...
			memcpy_s(&UniqueID, sizeof(CalibrationInfo), &CalibrationInfo, sizeof(CalibrationInfo));

			(UniqueID != 0); // if this fires, the calibration data didn't
	...
	while (1)
	{
			...
			!remote.IsPlayingAudio())
		{
			...
			if (!remote.IsBalanceBoard() &&
				...
				(remote.Internal.ExtensionType != NEW_MOTION_PLUS) &&
...
{
	(bytes_read == REPORT_LENGTH);

	// copy our input buffer
	BYTE buff[REPORT_LENGTH];
	memcpy_s(buff, bytes_read, ReadBuff, bytes_read);
...
void wiimote::SetReportType(input_report type, bool continuous)
{
	...
	(!IsBalanceBoard() || type == IN_BUTTONS_BALANCE_BOARD);
...
void wiimote::WriteData(int address, BYTE size, const BYTE* buff)
{
	...
	write[5] = size;
	memcpy_s(write + 6, size, buff, size);
	WriteReport(write);
}
// ------------------------------------------------------------------------------------
...
int wiimote::ParseReadAddress(BYTE* buff)
{
	...
	else if ((buff[3] & 0x07) != 0)
	{
		...
		{
			--MotionPlusDetectCount;
			if (Internal.ExtensionType == MOTION_PLUS || Internal.ExtensionType == NEW_MOTION_PLUS)
			{
				if (bMotionPlusDetected)
...
//					InitializeExtension();
	...
	case (REGISTER_EXTENSION_TYPE & 0xffff) :
	{
		...
		static const QWORD NEW_MOTION_PLUS = 0x050420A40001ULL;
		static const QWORD NEW_MOTION_PLUS_DETECT = 0x050020a60001ULL;
		static const QWORD NEW_MOTION_PLUS_DETECT2 = 0x050420a60001ULL;
		static const QWORD PARTIALLY_INSERTED = 0xffffffffffffULL;

		// MotionPlus: _before_ it's been activated
		if ((type == MOTION_PLUS_DETECT) || (type == MOTION_PLUS_DETECT2) || (type == NEW_MOTION_PLUS_DETECT) || (type == NEW_MOTION_PLUS_DETECT2))
		{
			if (!bMotionPlusDetected) {
...
else IF_TYPE(NEW_MOTION_PLUS)
TRACE(_T(".. Motion Plus! (WiiRemote Plus)"));
		// and start a query for the calibration data
		ReadAddress(REGISTER_EXTENSION_CALIBRATION, 16);
		bMotionPlusDetected = true;
	}
	...
	ReadAddress(REGISTER_EXTENSION_CALIBRATION, 16);
		...
		{
			//			(((Internal.ExtensionType == BALANCE_BOARD) && (size == 31)) ||
...
#endif
			memcpy_s(buff_copy, REPORT_LENGTH, buff, REPORT_LENGTH);
...
#define READ_SIZE(ptr,size)	if(fread(ptr, size, 1, file) != 1) {		   \
		...
		// valid RIFF file?
		(!strncmp(riff_chunkheader.ckID, "RIFF", 4));
		if (strncmp(riff_chunkheader.ckID, "RIFF", 4))
			goto unsupported; // nope
		// valid WAV variant?
		(!strncmp(riff_chunkheader.formType, "WAVE", 4));
		...
		while (1)
		{
			...
			else if (!strncmp(chunk_header.ckID, "data", 4))
			{
				...
				if (read != total_samples)
				{
					...
					total_samples = static_cast<unsigned int>(read);
				}
		...
		speaker_freq freq_override)
	{
		...
		BYTE bytes[7] = { (BYTE)0x00,(BYTE)0x00, (BYTE)0x00, (BYTE)(10 + (BYTE)freq), volume, (BYTE)0x00, (BYTE)0x00 };
		WriteData(0x04a20001, sizeof(bytes), bytes);
